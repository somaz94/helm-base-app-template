...
Common labels
*/}}
{{- define "base-azure.labels" -}}
helm.sh/chart: {{ include "base-azure.chart" . }}
{{ include "base-azure.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "base-azure.selectorLabels" -}}
app.kubernetes.io/name: {{ include "base-azure.name" . }}
...
*/}}
{{- define "base-azure.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "base-azure.fullname" .) .Values.serviceAccount.name }}
{{- else }}
...
{{- define "base-azure.podTemplate" }}
    metadata:
      annotations:
        ...
        {{- end }}
      labels:
        {{- include "base-azure.selectorLabels" . | nindent 8 }}
        {{- if .Values.version }}
    ...
    spec:
      ...
      imagePullSecrets:
        {{- toYaml . | nindent 8 }}
      {{- end }}
      serviceAccountName: {{ include "base-azure.serviceAccountName" . }}
      ...
      containers:
        - name: {{ include "base-azure.fullname" . }}
          ...
          {{- if $hasExternalSecrets }}
            ...
            - secretRef:
                name: {{ include "base-azure.fullname" . }}-secret
          {{- end }}
          {{- end }}
          ports:
            - name: http
          ...
          lifecycle:
            {{- toYaml . | nindent 12 }}
          {{- end }}
          {{- if .Values.storage.persistentVolumeClaims.enabled }}
          volumeMounts:
            {{- with .Values.volumeMounts }}
            {{- toYaml . | nindent 12 }}
            {{- end }}
            {{- range .Values.storage.persistentVolumeClaims.items }}
